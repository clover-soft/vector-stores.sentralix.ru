...
### 2025-12-17: Dry-run публикации и переиндексация (шаг 8)

- Цель:
  - Дать фронтенду режим предварительного просмотра операций публикации (attach/detach) без изменений в провайдере.
  - Дать явную операцию “обновить индекс” для UI.
- Изменения:
  - Расширен `POST /api/v1/indexes/{index_id}/publish`: добавлен параметр `dry_run` и типизированный ответ с планом операций.
  - Добавлен `POST /api/v1/indexes/{index_id}/reindex` как обёртка над publish (без ожидания индексации).

### 2025-12-17: Статусы файлов внутри индекса (шаг 9)

- Цель:
  - Дать фронтенду каноничный способ получать статусы файлов в рамках конкретного индекса без обращения к admin-ручкам.
- Изменения:
  - Добавлен сервис `IndexFilesProviderStatusService` для агрегации:
    - локальных файлов индекса (`rag_index_files` + `rag_files`)
